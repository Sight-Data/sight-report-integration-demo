...
打开 <http://localhost:3010/demo/>，按控制台上的 4 步走：

1. **连接后端** — 读取配置，确认能连上报表系统
2. **用户与报表** — 填 `account` / `userName`，点「加载列表」选一张报表
3. **参数与显示** — 配查询参数与显示开关，点「打开报表」（`⌘/Ctrl + ↵`）
4. **高级能力** — 事件订阅、宿主调用、外部数据集、服务端导出

右侧观察窗同时给出事件日志、embed URL、发现结果与完整请求日志——联调时看这里。
